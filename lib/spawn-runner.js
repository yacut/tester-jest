...
import { has, forEach, get, indexOf, remove, subtract, union } from 'lodash';
...
function parseReporterOutput(data/* :string*/, filePath/* :string*/, textEditor /* :TextEditor*/) {
  return new Promise((res, rej) => {
    ...
    const messages = [];
    if (has(data, 'testResults[0].assertionResults') && data.testResults[0].assertionResults.length > 0) {
      forEach(data.testResults[0].assertionResults, (testResult) => {
        textEditor.scan(new RegExp(testResult.title.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&')), (scanResult) => {
          ...
          messages.push({
            ...
            error: {
              ...
              message: toString(get(testResult, 'failureMessages')),
            },
            duration: subtract(get(data, 'testResults[0].endTime') - get(data, 'testResults[0].startTime')),
...
export function run(textEditor/* :TextEditor*/) {
  return new Promise((resolve) => {
    ...
    if (fileModified) {
      outputFilePath = `${dirname(fileName)}/.${basename(fileName)}`;
      writeFileSync(outputFilePath, textEditor.getText());
    }
    const jsonOutputFilePath = join(tmpdir(), `tester-jest_${basename(fileName)}.json`);
    ...
    let args = union(userConfigArgs, ['--json', '--outputFile', jsonOutputFilePath, outputFilePath]);
    ...
    const exit = () => {
      ...
      readFile(jsonOutputFilePath, 'utf8', (error, data) => {
        if (error) {
          ...
          parseReporterOutput(data, outputFilePath, textEditor)
