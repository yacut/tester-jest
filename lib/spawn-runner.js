    ...
    function parseReporterOutput(outputString) {
      ...
      _.forEach(_.split(outputString, os.EOL), (line) => {
        try {
          ...
          console.log('outputEvent', summary);
          if (_.has(summary, 'testResults') && summary.testResults.length > 0) {
            _.forEach(summary.testResults, (testResult) => {
              messages.push({
                state: testResult.status,
                title: _.get(testResult, 'assertionResults[0].title'),
                error: _.get(testResult, 'assertionResults[0].failureMessages[0]'),
