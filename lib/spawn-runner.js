...
export function getFormattedTesterMessage(
  state :TestState,
  title :string,
  errorMessage :string,
  duration :number,
...
export function getMessagesFromAssertionResults(
  ...
  return Promise.reduce(assertionResults, (messages, testResult) =>
    new Promise((resolve) => {
      textEditor.scan(
        new RegExp(testResult.title.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&')),
        (scanResult) => {
          ...
          duration = Math.round(duration / assertionResults.length);
...
export function getMessagesFromTestLocations(
  textEditor :TextEditor,
...
export async function parseReporterOutput(
  ...
  if (dataJSON && dataJSON.testResults && dataJSON.testResults.length > 0) {
    ...
    if (filePath && textEditor) {
      const { assertionResults } = dataJSON.testResults[0];
...
export function run(textEditor :?TextEditor, additionalArgs :?string) {
  return new Promise((resolve) => {
    ...
    if (atom.config.get('tester-jest.overrideNodePath')) {
      env.NODE_PATH = cwd;
    }
    ...
    this.bufferedProcess.onWillThrowError((errorObject) => {
      ...
      resolve({
        messages: [getFormattedTesterMessage('unknown', 'No results', errorObject.error.toString(), 0, 0, filePath)],
        output: errorPopup + errorObject.error.toString(),
      });
    });
  });
}
